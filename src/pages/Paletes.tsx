import { ArrowLeft, Package, Ruler, MapPin, Warehouse, AlertTriangle, Forklift, MessageCircle, Truck } from "lucide-react";
...
export default function Paletes() {
  ...
  const features = [
    { icon: Package, titleKey: "pal.f1_title" as const, descKey: "pal.f1_desc" as const },
    { icon: Ruler, titleKey: "pal.f2_title" as const, descKey: "pal.f2_desc" as const },
    { icon: MapPin, titleKey: "pal.f3_title" as const, descKey: "pal.f3_desc" as const },
    { icon: Warehouse, titleKey: "pal.f4_title" as const, descKey: "pal.f4_desc" as const },
  ];

  const notes = [
    { icon: Forklift, titleKey: "pal.note1_title" as const, descKey: "pal.note1_desc" as const },
    { icon: AlertTriangle, titleKey: "pal.note2_title" as const, descKey: "pal.note2_desc" as const },
  ];

  const waUrl = `https://wa.me/${OFFICE_WA}?text=${encodeURIComponent(t("pal.wa_text", lang))}`;
      ...
      <main className="max-w-4xl mx-auto px-4 sm:px-6 py-24 sm:py-32">
        ...
        <div className="relative rounded-2xl sm:rounded-3xl overflow-hidden mb-10 sm:mb-14 bg-gradient-to-br from-primary/15 via-surface to-surface p-8 sm:p-12">
          ...
          <div className="mt-8">
            ...
            <p className="text-xs sm:text-sm text-muted-foreground mt-3">{t("pal.cta_sub", lang)}</p>
          </div>
        </div>
        ...
        {/* Important notes */}
        <h2 className="text-xl sm:text-2xl font-extrabold mb-5 sm:mb-6">{t("pal.notes_title", lang)}</h2>
        ...
        <div className="text-center bg-surface rounded-2xl sm:rounded-3xl p-8 sm:p-12">
          <h2 className="text-2xl sm:text-3xl font-extrabold mb-3">{t("pal.cta", lang)}</h2>
          <p className="text-muted-foreground text-sm sm:text-base mb-6">{t("pal.cta_sub", lang)}</p>
          <a
            href={waUrl}
            target="_blank"
            rel="noopener noreferrer"
